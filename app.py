...
with tab3:
    ...
    .nav-footer { display:flex; gap:12px; margin-top:20px; }
    .nav-btn { flex:1; display:flex; align-items:center; justify-content:center; gap:10px;
               padding:14px 20px; background:#FFFFFF; border:1.5px solid #D0BCAD;
               border-radius:10px; font-size:0.88rem; font-weight:600; color:#465030; }
    @media print {
        [data-testid="stSidebar"],[data-testid="stHeader"],[data-testid="stToolbar"],
        [data-testid="stDecoration"],[data-testid="stStatusWidget"],
        .stButton,.stSlider,.stSelectbox,footer,.nav-footer,.no-print,
        .meta-actions,.action-divider,a.action-btn { display:none !important; }
        @page { size:A4 portrait; margin:1.2cm 1.4cm; }
        html,body { background:#FFFFFF !important; color:#2C1810 !important; font-size:9pt !important; }
        .stApp,[data-testid="stAppViewContainer"],[data-testid="stMain"],
        [data-testid="block-container"],.main .block-container,section.main {
    ...
    _t_start = pd.Timestamp(timestamps[0])
    _t_end   = pd.Timestamp(timestamps[-1])
    ...
    _total_occ_final  = int(_df_final["DP_Total_Occupancy"].sum())
    _total_vuln_final = int(_df_final["DP_Vulnerable_Count"].sum())
    ...
    _alarm_count_total= int(df_scenario["Fire_Alarm_Triggered"].sum()) if "Fire_Alarm_Triggered" in df_scenario.columns else 0
    _peak_occ         = int(df_scenario.groupby("Timestamp")["Total_Occupancy_Raw"].sum().max()) if "Total_Occupancy_Raw" in df_scenario.columns else 0
    _sprinkler_on     = bool("Sprinkler_System_Status" in df_scenario.columns and df_scenario["Sprinkler_System_Status"].eq("Activated").any())

    if "Predicted_Hazard_Score" in df_scenario.columns:
        _highest_hz_zone = str(df_scenario.groupby("Zone_ID")["Predicted_Hazard_Score"].mean().idxmax())
    elif "Hazard_Level_Rating" in df_scenario.columns:
        _highest_hz_zone = str(
            df_scenario.groupby("Zone_ID")["Hazard_Level_Rating"]
            .apply(lambda s: _HAZARD_ORDER.get(s.mode().iloc[0] if not s.empty else "Low", 0))
            .idxmax()
        )
    else:
    ...
    _inc_type     = "Fire Emergency" if is_fire else "Routine Monitoring"
    ...
    st.markdown(
        ...
        "background:#465030;color:#fff;border-radius:6px;"
        "padding:9px 20px;font-size:0.82rem;font-weight:600;cursor:pointer;"
        "font-family:DM Sans,sans-serif;text-decoration:none;margin-bottom:10px;'>"
        + _T3["print"] + " Print Report</a>",
        unsafe_allow_html=True,
    )
    ...
    with _bot_r:
        st.markdown(
            ...
            "<a class='action-btn action-btn-primary' href='javascript:void(0);' onclick='window.print();return false;'>"
            + _T3["print"] + " Print / Save PDF</a>"
            ...
            "<b>Print tip:</b> Use "
            "<kbd style='background:#EDE5DC;padding:1px 5px;border-radius:3px;font-size:0.75rem;'>Ctrl+P</kbd>"
            " or the button above to save as PDF. Sidebar and controls are hidden automatically."
            "</div></div>",
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        "<div class='nav-btn'>" + _T3["overview"] + " &nbsp; Overview</div>"
        "<div class='nav-btn'>" + _T3["building"] + " &nbsp; Zone Details</div>"
        "<div class='nav-btn' style='background:#E5ECDE;border-color:#AACBA0;'>"
        + _T3["report"] + " &nbsp; Reports</div></div>",
        unsafe_allow_html=True,
    )
