...
ZONES = [
    Zone("Z1", "Reception", 10, 6.0, 12.0, 34.0, 44.0),
    Zone("Z2", "Workspace", 30, 36.0, 12.0, 64.0, 44.0),
    Zone("Z3", "MeetingRoom", 12, 66.0, 12.0, 94.0, 44.0),
    Zone("Z4", "Pantry", 8, 6.0, 56.0, 34.0, 88.0),
    Zone("Z5", "Corridor", 15, 36.0, 56.0, 64.0, 88.0),
    Zone("Z6", "ExitArea", 5, 66.0, 56.0, 94.0, 88.0),
]
...
def zone_center(zone: Zone) -> tuple[float, float]:
    return (
        round((zone.zone_x_min + zone.zone_x_max) / 2, 2),
        round((zone.zone_y_min + zone.zone_y_max) / 2, 2),
    )


def evacuation_progress(ts: datetime, scenario: Scenario) -> float:
    if not scenario.fire_start_time or ts < scenario.fire_start_time:
        return 0.0

    elapsed = (ts - scenario.fire_start_time).total_seconds()
    total = max((scenario.end_time - scenario.fire_start_time).total_seconds(), 1)
    return clamp01(elapsed / total)


def cluster_count(total_occ: int) -> int:
    if total_occ <= 0:
        return 0
    if total_occ <= 4:
        return 1
...
def relative_cluster_point(
    ...
    base_x, base_y = CLUSTER_ANCHORS[anchor_index % len(CLUSTER_ANCHORS)]
    progress = evacuation_progress(ts, scenario)
    exit_center_x, exit_center_y = zone_center(ZONE_LOOKUP["Z6"])
    zone_center_x, zone_center_y = zone_center(zone)

    x_direction = 0.0
    if exit_center_x > zone_center_x:
        x_direction = 1.0
    elif exit_center_x < zone_center_x:
        x_direction = -1.0

    y_direction = 0.0
    if exit_center_y > zone_center_y:
        y_direction = 1.0
    elif exit_center_y < zone_center_y:
        y_direction = -1.0

    if progress > 0:
        base_x = clamp(base_x + (0.12 * progress * x_direction), 0.15, 0.85)
        base_y = clamp(base_y + (0.1 * progress * y_direction), 0.15, 0.85)

    if route_passable == "No":
        base_x = clamp(base_x - (0.08 * progress), 0.18, 0.82)
        base_y = clamp(base_y - (0.05 * progress), 0.18, 0.82)
    elif route_passable == "Partial":
        base_x = clamp(base_x - (0.04 * progress), 0.16, 0.84)

    return base_x, base_y


def absolute_cluster_point(
    zone: Zone,
    relative_x: float,
    relative_y: float,
) -> tuple[float, float]:
    zone_width = zone.zone_x_max - zone.zone_x_min
    zone_height = zone.zone_y_max - zone.zone_y_min

    coord_x = zone.zone_x_min + (zone_width * relative_x) + np.random.normal(0, zone_width * 0.03)
    coord_y = zone.zone_y_min + (zone_height * relative_y) + np.random.normal(0, zone_height * 0.03)

    return (
        round(clamp(coord_x, zone.zone_x_min + 0.5, zone.zone_x_max - 0.5), 2),
        round(clamp(coord_y, zone.zone_y_min + 0.5, zone.zone_y_max - 0.5), 2),
    )
...
def build_cluster_rows(
    ...
    total_clusters = cluster_count(total_occ)
    if total_clusters == 0:
        return []

    weights = np.random.dirichlet(np.ones(total_clusters))
    cluster_sizes = np.random.multinomial(total_occ, weights)
    rows = []
    cluster_number = 1

    for idx, cluster_size in enumerate(cluster_sizes, start=1):
        if cluster_size <= 0:
            continue

        relative_x, relative_y = relative_cluster_point(
            zone=zone,
            scenario=scenario,
            ts=ts,
        ...
        coord_x, coord_y = absolute_cluster_point(zone, relative_x, relative_y)
        rows.append({
            ...
            "Cluster_ID": f"{row_incident_id}-{zone.zone_id}-C{cluster_number}",
            "Coord_X": coord_x,
            "Coord_Y": coord_y,
...
def generate_dataset() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        for ts in timestamps:
            for zone in ZONES:
                ...
                rows.append({
                    ...
                    "Zone_ID": zone.zone_id,
                    "Zone_Name": zone.zone_name,
                    "Zone_X_Min": zone.zone_x_min,
                    "Zone_Y_Min": zone.zone_y_min,
                    "Zone_X_Max": zone.zone_x_max,
                    "Zone_Y_Max": zone.zone_y_max,
                ...
                cluster_rows.extend(
                    build_cluster_rows(
                        ts=ts,
                        row_incident_id=row_incident_id,
                        scenario=scenario,
                        zone=zone,
                        total_occ=total_raw,
                        route_passable=route_passable,
    ...
    cluster_df = pd.DataFrame(cluster_rows, columns=[
        "Timestamp",
        "Incident_ID",
        "Building_Level",
        "Zone_ID",
    ...
    return df[final_columns], df[enriched_columns], cluster_df
    ...
    for level in building_levels:
        for zone in ZONES:
            rows.append({
                ...
                "Zone_X_Max": zone.zone_x_max,
                "Zone_Y_Max": zone.zone_y_max,
                "Zone_Center_X": zone_center(zone)[0],
                "Zone_Center_Y": zone_center(zone)[1],
...
if __name__ == "__main__":
    final_dataset, enriched_dataset, cluster_dataset = generate_dataset()
    ...
    save_dataset(enriched_dataset, "data/emergency_zone_data.csv")
    save_dataset(cluster_dataset, "data/occupancy_clusters.csv")
